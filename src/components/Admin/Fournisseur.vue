<template>
  <div>
    ...
    <!-- body fournisseur -->
    <div class="categorieContainer" style="width:80%">
      ...
      <div class="containerCategorie">
        ...
        <div id="modifyFournisseur" class="modal-vue">
          ...
          >
            ...
            <p>
              <label for="" style="float:left;">Modifier le nom du fournisseur</label>
              <input
                type="text"
                name="fournisseur"
            ...
            <p>
              <label for="" style="float:left;">Modifier le téléphone du fournisseur</label>
              <input
                type="text"
                name="fournisseur"
  ...
  methods: {
    ...
    async deleteCategory() {
      ...
      if (permission.status === 200) {
        var result = await toDelete("fournisseurs/" + this.currentCategorieId);
    ...
    async updateCategory(event) {
      ...
      try {
        ...
        const mod = await update("fournisseurs/" + this.currentCategorieId, this.newFournisseur);
        console.log(mod)
        if (mod.status === 201) {
          this.isSuccess = true;
    ...
    filterData(value) {
      this.filteredList = this.fournisseurs.filter((fournisseur) => {
        return (fournisseur.libelle.toLowerCase().trim().includes(value.toLowerCase()) || fournisseur.telephone.toLowerCase().trim().includes(value.toLowerCase()));
      });
    },
    selectAll($event) {
