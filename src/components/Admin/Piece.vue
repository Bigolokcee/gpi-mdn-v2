    ...
    <div class="containerMateriel">
      ...
      <div id="showForm" class="modal-vue">
        ...
        <form v-if="showForm" @submit="handleSubmit" class="formMaterielAdd">
          ...
          <h1 class="h1">AJOUTER UNE PIECE</h1>
          ...
          <div class="divForm">
            ...
            <div class="divForm2">
              ...
              <p>
                <button class="button">Ajouter une piece</button>
              </p>
            </div>
          </div>
        </form>
      ...
      <div id="modifyMateriel" class="modal-vue">
        ...
        >
          ...
          <h1 class="h1">MODIFIER UNE PIECE</h1>
          <!--   <div :class="isSuccess ? 'success' : 'error'" v-if="responsePost != ''">
            {{ responsePost }} -->
          <div class="divForm">
            <div class="divForm1">
      ...
      <div id="deleteMateriel" class="modal-vue">
        ...
        <form v-if="deleteMat" @submit="handleSubmit" class="formMateriel">
          ...
          <div>
            ...
            <p for="">Êtes-vous sûr de vouloir supprimer cette piece ?</p>
          </div>
          <div>
    ...
    data() {
      ...
      return {
        ...
        materielForm: {
          ...
          plusIconeTitle: 'Ajouter une piece',
          class: {
            search: 'fa fa-search',
    ...
    methods: {
      ...
      async handleSubmit(event) {
        ...
        if (this.materiel.libelle === '') {
          ...
          await post('pieces', this.materiel);
      ...
      async updateMateriel() {
        if (this.updateM.fonction == '' || this.updateM.categorie == '') {
          ...
          const res = await update(
            'pieces/' + this.currentMaterielId,
            this.updateM
          );

          if (res.status == 201) {
            displayMessage('P-Upd');
          } else {
            displayMessage(
              'Oops',
      ...
      async deleteMateriel(e) {
        ...
        if (permission.status == 200) {
          await toDelete('pieces/' + this.currentMaterielId);
          displayMessage('P-Del');
          this.deleteMat = true;
          this.loadMateriels('pieces/' + this.currentMaterielId);
        } else {
          displayMessage('P-Auth');
        }
      },
      async loadMateriels() {
        ...
        if (this.userStatut === 'root') {
          ...
          const response = await load('pieces/all');
      ...
      async loadData() {
        if (this.userStatut === 'root') {
          ...
          const resultBureaux = await load(
            'fonctions/all?directionId=' + directionId
          );
          this.fonctions = resultBureaux.data;
          this.newData = this.fonctions.map((elt) => {
            return { libelle: elt.libelle, id: elt._id };
      ...
      async deleteSelectedElement() {
        ...
        if (permission.status == 200) {
          for (var checkbox of this.selectedCheckbox) {
            await toDelete('pieces/' + checkbox);
          }
          displayMessage('P-Del');
          this.nbrOfTaskSelected = 0;
          this.isSubHeaderActive = false;
