...
<!--     {{ closeTrierBy() }} -->
    ...
    <div
      ...
      <div class="divBtnTrieAndAdd" v-if="connectedUserStatut === 'root'">
       
      </div>
        <div class="divBtnTrieAndAdd">
          <i
            @click="showFormF"
            v-if="isForProblems === false || isForUser == false || connectedUserStatut != 'root'"
    ...
    <hr style="margin-top: 5px; margin:auto; width: 95%;" >
     <div class="sortContainer" v-if="isOpen" :class="{'swing-in-top-fwd': isOpen}">
          <Select @selectLi="selectFilter" :options="options" defaultLabel="Trier selon un attribut"/>
          <Select v-if="connectedUserStatut == 'root'" @selectTutelle="selectedTutelle" :tutelles="tutelles" defaultLabel="Trier selon une tutelle"/>
          <Select @selectLi="selectFilter" :materielOptions="options" defaultLabel="Trier selon un attribut" v-if="materiel"/>
          <div class="sortAlphabetic">
            <img :src="require(`../../assets/${url}`)" @click ="sortInAlphabeticOrder" class="swing-in-top-fwd">
          </div>
    </div>
  </div>
</template>
...
export default {
  ...
  toSortOut: ['Tutelle'],
  props: ["options","entity","isForProblems","nbrOfTask","needSearchBar","radios","checkboxes","form","data","isSubHeaderActive","selected","arrayCheckbox","isForUser","materiel"],
  methods: {
