          ...
          <div class="message-body">
            <div class="problem">
              <li> 
                ...
                <p style="margin-top:10px;">{{ p.executeBy.prenom}} {{ p.executeBy.nom}}</p>
              </li> 
              <li>
              ...
              <li>
                  ...
                  <span v-if = "p.solutionPreconise.length != 0">
                    <ul v-for="(solution, index) in p.solutionPreconise" :key="index"> 
                      <li class="listSolutions">{{ solution }}</li> 
                    </ul>
                    <textarea v-if = "p.solutionPreconise.length % 2 != 0 && p.statut == 'false'" rows="1" type="text" class="textarea sp" placeholder="ENTRER LA SOLUTION PRECONISEE">
                    </textarea>
                    
                  <div>
                    <button  v-if = "p.statut == 'false'" class="button" @click = "sendDiagnostique(p)" >Envoyer Le Diagnostique</button> 
                  </div>
                  </span> 
                  <p v-else>
              ...
              <li v-if = "p.solutionPreconise.length % 2 != 0">
                <button @click = "approuve(p)" class="button">APPROUVER LA SOLUTION</button>
              </li>
            </div>
          </div>
        </article>
...
export default {
  ...
  methods:{
    ...
    async sendDiagnostique(p){
      ...
      if( sp === "") {
        ...
        if(res.status === 201){
            alert("Votre solution preconisé a été envoyé. Attendre la réponse de " + p.executeBy.nom + " " + p.executeBy.prenom)
        }
  ...
  computed: {
    showPbClass: function() {
      const openClasses = [];
      this.isOpen.forEach(it => openClasses.push({
        "is-closed": !it,
        "is-primary": it,
        "is-dark": it,
      }))
  ...
  filters: {
    dateElapsed: function (value) {
     // console.log(value)
      ...
      if ( years != 0) {
       return years == 1 || months == 1 ? `il y a ${months} année ${months} mois` : `il y a ${years} années ${months} mois`;

      }else if (years == 0 && months != 0 ) {
          return days > 1 ? `il y a ${months} mois ${days} jours` : `il y a ${months} mois ${days} jour`;

      } else if ( years == 0 && months == 0 && days != 0) {
          return days > 1 ? `il y a ${days} jours ${minutes} min` : `il y a ${months} mois ${days} jour`;

      }else if ( years == 0 && months == 0 && days == 0 && hours != 0) {
          return hours == 1 ? `il y a ${hours} heure ${minutes} min` : `il y a ${hours} heures ${minutes} min` ;
          
      }else if ( years == 0 && months == 0 && days == 0 && hours == 0 && minutes != 0) {
          return `il y a ${minutes} min ${seconds} s`

      }else if ( years == 0 && months == 0 && days == 0 && minutes == 0 && seconds != 0) {
    ...
    timeOfReception(value){
       ...
       var time = value.split('T')[1].slice(0, value.split('T')[1].length - 1).split(':');
       var heure = time[0]
       var min = time[1]
