...
async function all(req, res) {
    try {
        ...
        var executeBy = req.query.executeBy;
        var assignedTo = req.query.assignedTo;
        var solutionPreconise = req.query.solutionPreconise;
        var tutelle = req.query.tutelle;
        var isProgress = req.query.isProgress;
        ...
        var data = await Problem.find({ enabled: true,assignedTo, executeBy, tutelle}).populate({
            path: 'sender',
            populate: { path: 'direction'}
        }).populate('assignedTo').populate('executeBy').populate('tutelle').populate('materiel').sort({ createdAt: -1 })

        if (assignedTo == undefined && executeBy != undefined) {

            //Pour avoir les taches éffectué par le technicien
            data = await Problem.find({ enabled: true, tutelle }).populate({
                path: 'sender',
                populate: {
                    path: 'direction'
                }
            }).populate('assignedTo').populate('executeBy').populate('materiel');
        }
        ...
        if (executeBy != undefined && solutionPreconise != undefined) {
            ...
            data = await Problem.find({ enabled: true,tutelle, statut: { $ne: true } }).populate({
                path: 'sender',
                populate: {
                    path: 'direction'
                }
        ...
        if (solutionPreconise == 'true' && assignedTo != undefined) {
            ...
            data = await Problem.find({ enabled: true, assignedTo, tutelle }).populate({
                path: 'sender',
                populate: {
                    path: 'direction'
                }
        ...
        if (solutionPreconise == undefined && executeBy != undefined && assignedTo == undefined) {
            data = await Problem.find({ enabled: true, tutelle }).populate({
                path: 'sender',
                populate: {
                    path: 'direction'
        ...
        if (isProgress != undefined ) {
            ...
            //Pour avoir les taches suivies par le Chef de division
            data = await Problem.find({ enabled: true,  tutelle, isProgress }).populate({
                path: 'sender',
                populate: {
                    path: 'direction'
                }
...
/* 
       ...
       try {
         ...
         data = await Problem.find({ enabled: true,assignedTo, executeBy, tutelle}).populate({
            path: 'sender',
            populate: { path: 'direction'}
        }).populate('assignedTo').populate('executeBy').populate('tutelle').populate('materiel').sort({ createdAt: -1 })
       } catch (error) {
        ...
        if (assignedTo == undefined && executeBy != undefined) {
            ...
            //Pour avoir les taches éffectué par le technicien
            data = await Problem.find({ enabled: true, executeBy, tutelle }).populate({
                path: 'sender',
                populate: {
                    path: 'direction'
                }
        ...
        if ( assignedTo != undefined && executeBy == undefined ) {
            console.log("143")
            try {
                data = await Problem.find({ enabled: true,executeBy, assignedTo, tutelle }).populate({
                    path: 'sender',
                    populate: {
                        path: 'direction'
                    }
        ...
        if (executeBy != undefined && solutionPreconise == undefined) {
            console.log("158")
            solutionPreconise = null;
            data = await Problem.find({ enabled: true, executeBy, solutionPreconise, tutelle }).populate({
                path: 'sender',
                populate: {
                    path: 'direction'
                }
        ...
        if (executeBy != undefined && solutionPreconise != undefined) {
            console.log("169")
            data = await Problem.find({ enabled: true, executeBy, solutionPreconise, tutelle, statut: { $ne: true } }).populate({
                path: 'sender',
                populate: {
                    path: 'direction'
                }
        ...
        if (solutionPreconise == undefined && executeBy != undefined && assignedTo == undefined) {
            console.log("189")
            data = await Problem.find({ enabled: true, tutelle, assignedTo }).populate({
                path: 'sender',
                populate: {
                    path: 'direction'
                }
...
async function updateSomething(req, res) {
    try {
        ...
        const { solutionPreconise, statut, isProgress } = body;
...
export default {
    all, one, create, update, deleteAll,findByTutelle, updateSomething
}